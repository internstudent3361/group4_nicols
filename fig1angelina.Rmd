...
data1 <- data1 %>% 
  mutate(as.factor(cond))

data1$cond[data1$cond==4] <- "Religious" # Make religious prime the reference category
data1$cond[data1$cond==1] <- "Control" # This is in a weird order as R reads the code line by line, so if we go from top to bottom, 
data1$cond[data1$cond==3] <- "Secular" # we're changing the number twice which screws up our dataframe
data1$cond[data1$cond==4] <- "Religious"
data1$cond[data1$cond==2] <- "Noise"
...
data1 <- data1 %>% 
  mutate(numberOf = (cond == "Religious") * 100 + (cond == "Secular" | cond == "Noise") * 103 + (cond == "Control") * 102) %>% 
...
=======
**** Jenny comments ****
tips for getting your colors to work:
- make sure both data1$cond and group_cond$cond are factors (e.g. data1$cond <- factor(data1$cond, labels = c("Control","Noise","Secular","Religious")) ) 
- make sure this is set before you call your ggplot
- I'm not sure what scale_x_continuous is doing here, but if you change these two variables to factors, I don't think you'll need these this function anymore
```{r}
>>>>>>> 82ce9bdd29c2dc4fe4411811c69dcb0257cec6ca
# data by condition

# change the two variables to factors here:
...
=======
  scale_x_continuous(name = NULL, ## I think you should be able to delete or comment these lines (188-192) out
                     expand = c(0,0),
                     breaks = c(0, 1, 2, 3),
