...
Moving on from this, we needed to name the columns appropriately with M, SD, lCI, uCI and d - similar to the original table. This was completed using the function cols_label, where the original name of the column is renamed using the speech marks ("").
...
We needed to ensure that all values were rounded to two decimal places. This was achieved using fmt_number on columns with numeric values. This was difficult for values that were marked with a "-" initially, as they were not originally calculated by the researchers. By coming back to the original table visualisation and changing the "-" into NA, the function fmt_number finally worked. This was applied to each column and use_seps was set to false as we did not want any digit group separators. The exact function is: fmt_number(columns = ..., decimals = 2, use_seps = FALSE).
...
Finally, the very last thing to do was hide columns that repeated the %%claimed, sacredness, negativity, positivity, tempo and impact characteristics. This was repeated as each group initially had their own separate table. This was achieved using the function cols_hide(columns = c(...)).
